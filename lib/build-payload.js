...
// [COMPAT] The v1 `iOS Actionable Notification` subflow put message overrides at
// the TOP LEVEL of msg.notificationOverride (msg.notificationOverride.title),
// which is also how this builder already reads `tag`. Flows migrating to this
// node keep emitting that shape from their "Set Message" functions, so accept it
// as a fallback for the notificationBase fields. An explicit notificationBase
// always wins; keys that are not base fields (services, tag, actions, media,
// map) are deliberately excluded — they are handled separately.
const NOTIFICATION_BASE_KEYS = [
  'title', 'subtitle', 'message', 'url', 'cameraEntity', 'interruptionLevel',
  'customSound', 'group', 'populateUserInformation', 'clearNotificationsOnAction',
  'badge', 'categoryName', 'presentationOptions',
];

// Media fields are read from override.media, but the same v1 flows put these at
// the top level too (e.g. notificationOverride.imagePath from a Frigate
// "Set Message"). Same fallback, same precedence.
const MEDIA_KEYS = [
  'contentUrl', 'contentType', 'imagePath', 'videoPath', 'audioPath',
  'lazyLoading', 'hideThumbnail',
];

function pickLegacy(source, keys) {
  const out = {};
...
function buildNotificationPayloads(config, override) {
  ...
  const payloads = services.map((service) => {
    ...
    const ovNb = {
      ...pickLegacy(safeOverride, NOTIFICATION_BASE_KEYS),
      ...(safeOverride.notificationBase || {}),
    };
    ...
    const ovMd = {
      ...pickLegacy(safeOverride, MEDIA_KEYS),
      ...(safeOverride.media || {}),
    };
